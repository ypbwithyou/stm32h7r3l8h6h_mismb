...
#define FRAME_MIN_SIZE (FREAM_HEAD_LENGTH + FREAM_LEN_LENGTH + FREAM_CHECK_LENGTH + FREAM_END_LENGTH)
// tag length
...
enum EventsId
{
    ...
    DVS_INIT_ZEROING = (EVTSEG_INIT + 73),
    DVS_INIT_ZEROING_OK = (EVTSEG_INIT + 74),
...
{
    double minimum;              // 针对AoRecLocalColumn-最小Y值,最新记录结构只有LocalColumnX
    double maximum;              // 针对AoRecLocalColumn-最大Y值
    int measurement_quantity;    // 针对AoRecLocalColumn-最小值X Index
    int raw_datatype;            // 数据类型，0：int，1：FLOAT，2：DOUBLE，3：COMPLEX。	//针对AoRecLocalColumn-最大值X Index
    unsigned long long nNanoSec; // 64bit 8字节
    double gp0;                  // 预留参数1
...
} AoLocalColumn, AoRecLocalColumn;
